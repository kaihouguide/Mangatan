// ==UserScript==
// @name         Automatic Content OCR (Local Overlay Manager - v7.1 Final Focus Fix)
// @namespace    http://tampermonkey.net/
// @version      7.1
// @description  The definitive version. Combines the flicker-free wrapper with the restored focus-highlighting logic and full click-through capability.
...
(function() {
    ...
    function activateScanner() {
        logDebug("Activating scanner v7.1...");
    ...
    // --- DISPLAY LOGIC (Wrapper-based with Focus) ---
    function displayOcrResults(targetImg) {
        ...
        // Event listeners on the WRAPPER to prevent flicker
        wrapper.addEventListener('mouseenter', () => {
            overlayContainer.classList.add('is-visible');
        });
        wrapper.addEventListener('mouseleave', () => {
            overlayContainer.classList.remove('is-visible');
            overlayContainer.classList.remove('is-focused'); // Clean up focus
            const currentlyFocused = overlayContainer.querySelector('.focused');
            if(currentlyFocused) currentlyFocused.classList.remove('focused');
        });

        // Event delegation on the WRAPPER for focus highlighting
        wrapper.addEventListener('mouseover', (e) => {
            if (e.target.classList.contains('gemini-ocr-text-box')) {
                const currentlyFocused = overlayContainer.querySelector('.focused');
                if (currentlyFocused && currentlyFocused !== e.target) {
    ...
    async function createUI() {
        GM_addStyle(`
            ...
            .gemini-ocr-image-wrapper { position: relative; display: block; line-height: 0; }
            .gemini-ocr-phantom-overlay { position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 9999; visibility: hidden; opacity: 0; transition: opacity 0.2s ease-in-out, visibility 0s 0.2s; pointer-events: none; }
            .gemini-ocr-phantom-overlay.is-visible { visibility: visible; opacity: 1; transition-delay: 0s; }
            .gemini-ocr-text-box { position: absolute; background: rgba(10, 25, 40, 0.6); border: 2px solid rgba(0, 191, 255, 0.7); color: white; text-shadow: 1px 1px 2px #000, -1px -1px 2px #000, 1px -1px 2px #000, -1px 1px 2px #000; backdrop-filter: blur(1.5px); line-height: 1.25; display: flex; align-items: center; justify-content: center; text-align: center; border-radius: 3px; box-sizing: border-box; user-select: text; cursor: text; overflow: visible; pointer-events: auto; transition: opacity 0.2s, transform 0.2s; }
            ...
            .gemini-ocr-phantom-overlay.is-focused .gemini-ocr-text-box.focused { opacity: 1; transform: scale(1.02); }
        ...
        modal.innerHTML = `
            <h2 class="full-width">Local OCR Settings (v7.1)</h2>
            <div class="gemini-ocr-settings-grid">
                ...
                <div class="full-width">
                    <label for="gemini-ocr-sites-config">Site Configurations (Format: URL; OverflowFix; Container1; Container2...)</label>
        ...
        const loadedSettings = await GM_getValue('gemini_ocr_observer_settings_v7.1');
        ...
        document.getElementById('gemini-ocr-save-btn').addEventListener('click', async () => {
            ...
            await GM_setValue('gemini_ocr_observer_settings_v7.1', JSON.stringify(newSettings));

// ==UserScript==
// @name         Automatic Content OCR (Local Overlay Manager - v21.5.16)
// @namespace    http://tampermonkey.net/
// @version      21.5.16
// @description  Fixes highlight text contrast for color themes. Adds "Adaptive Font" and "Server Angle" options. Overlays appear on hover, highlighting is configurable.
// @author       1Selxo (with modifications by Gemini)
...
(function() {
    ...
    // --- Color Themes (v2 - Contrast Fixed) ---
    const COLOR_THEMES = {
    ...
    const logDebug = (message) => {
        ...
        console.log(`[OCR v21.5.16] ${logEntry}`);
    ...
    function activateScanner() {
        logDebug("Activating scanner v21.5.16...");
    ...
    // --- DECOUPLED OVERLAY ENGINE with HIGHLIGHT MODES (v21.5.11) ---
    function displayOcrResults(targetImg) {
        ...
        data.forEach((item) => {
            ...
            switch (settings.textOrientation) {
                case 'forceVertical':
                    isVertical = true;
                    break;
                case 'smart':
                    isVertical = item.tightBoundingBox.height > item.tightBoundingBox.width;
                    break;
                case 'serverAngle':
                    isVertical = item.orientation === 90;
                    break;
            }
    ...
    function manageUrlBasedStyles() {
        const htmlEl = document.documentElement;
        if (window.location.href.includes('/chapter/')) {
            if (!htmlEl.classList.contains('ocr-scroll-fix-active')) {
                htmlEl.classList.add('ocr-scroll-fix-active');
                logDebug("Scroll fix applied for /chapter/ URL.");
            }
        } else {
            if (htmlEl.classList.contains('ocr-scroll-fix-active')) {
                htmlEl.classList.remove('ocr-scroll-fix-active');
                logDebug("Scroll fix removed.");
            }
        }
    }
    ...
    function updateAllOverlays() {
        ...
        try {
            ...
            for (const [img, state] of managedElements.entries()) {
                ...
                // --- FONT SIZE LOGIC (ADAPTIVE OR GLOBAL) ---
                if (settings.adaptiveFont) {
                    state.overlay.querySelectorAll('.gemini-ocr-text-box').forEach(box => {
                        ...
                        let sizeFromHeight;
                        if (isVertical) {
                            sizeFromHeight = (boxRect.height * scalingFactor) / text.length;
                            sizeFromWidth = boxRect.width * scalingFactor;
                        } else {
                            sizeFromWidth = (boxRect.width * scalingFactor) / (text.length * charAspectRatio);
                            sizeFromHeight = boxRect.height * scalingFactor;
                        }
                        const finalFontSize = Math.min(sizeFromWidth, sizeFromHeight) * 0.85;
                        box.style.fontSize = `${Math.max(finalFontSize, minFontSize)}px`;
                    });
                } else {
    ...
    function createUI() {
        GM_addStyle(`
            /* Dynamic scrollbar fix for reader view */
            ...
            .gemini-ocr-decoupled-overlay { position: absolute; z-index: 9999; pointer-events: none !important; transition: opacity 0.15s ease-in-out, visibility 0.15s ease-in-out; }
            .gemini-ocr-decoupled-overlay.is-hidden { opacity: 0; visibility: hidden; }
            .gemini-ocr-text-box {
                position: absolute; background: var(--ocr-bg-color);
                border: 2px solid var(--ocr-border-color); color: var(--ocr-text-color);
            ...
            .interaction-mode-hover.is-focused .gemini-ocr-text-box:hover {
                opacity: 1; transform: scale(1.05); background: var(--ocr-highlight-bg-color);
                border-color: var(--ocr-highlight-border-color); color: var(--ocr-highlight-text-color); text-shadow: none;
                box-shadow: var(--ocr-highlight-shadow); z-index: 10000;
            }
            ...
            .interaction-mode-click.is-focused .manual-highlight {
                opacity: 1; transform: scale(1.05); background: var(--ocr-highlight-bg-color);
                border-color: var(--ocr-highlight-border-color); color: var(--ocr-highlight-text-color); text-shadow: none;
                box-shadow: var(--ocr-highlight-shadow), var(--ocr-highlight-inset-shadow); z-index: 10000;
            }
            ...
            .gemini-ocr-modal h2 { margin: 0; color: var(--modal-header-color); } .gemini-ocr-modal h3 { font-size: 1.1em; margin: 15px 0 10px 0; border-bottom: 1px solid #333; padding-bottom: 5px; color: var(--modal-header-color); }
            ...
            .gemini-ocr-modal input:disabled { background-color: #444; color: #888; cursor: not-allowed; }
        ...
        document.body.insertAdjacentHTML('beforeend', `
            ...
            <div id="gemini-ocr-settings-modal" class="gemini-ocr-modal is-hidden">
                <div class="gemini-ocr-modal-header"><h2>Local OCR Settings (v21.5.16)</h2></div>
                <div class="gemini-ocr-modal-content">
                    ...
                    <h3>Interaction & Display</h3><div class="gemini-ocr-settings-grid">
                        ...
                        <label for="ocr-adaptive-font" title="Overrides Font Size % and sizes font to fit on a single line.">Adaptive Font:</label><input type="checkbox" id="ocr-adaptive-font">
                    </div>
    ...
    function bindUIEvents() {
        Object.assign(UI, {
            ...
            fontSizeInput: document.getElementById('ocr-font-size'),
            adaptiveFontCheckbox: document.getElementById('ocr-adaptive-font'),
        ...
        UI.adaptiveFontCheckbox.addEventListener('change', () => {
            UI.fontSizeInput.disabled = UI.adaptiveFontCheckbox.checked;
        });

        UI.colorThemeSelect.addEventListener('change', () => {
            document.documentElement.style.setProperty('--modal-header-color', (COLOR_THEMES[UI.colorThemeSelect.value] || COLOR_THEMES.deepblue).main + '1)');
        });
        ...
        UI.saveBtn.addEventListener('click', async () => {
            const newSettings = {
                ...
                adaptiveFont: UI.adaptiveFontCheckbox.checked,
    ...
    async function init() {
        ...
        UI.adaptiveFontCheckbox.checked = settings.adaptiveFont;
        UI.fontSizeInput.disabled = settings.adaptiveFont;
    ...
    init().catch(e => console.error(`[OCR v21.5.16] Fatal Initialization Error: ${e.message}`));
})();

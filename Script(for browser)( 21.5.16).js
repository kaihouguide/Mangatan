// ==UserScript==
// @name         Automatic Content OCR (Local Overlay Manager - v21.5.19+ScrollFix)
// @namespace    http://tampermonkey.net/
// @version      21.5.20
// @description  Integrated dynamic scroll fix for '/manga/' URLs and major performance refactor. Font-sizing is now cached and only recalculates on dimension changes.
// @author       1Selxo 
...
(function() {
    ...
    // ManagedElements tracks { overlay, hideTimeout, lastWidth, lastHeight }
    ...
    let measurementSpan = null; // A single, reused element for performant text measurement
    const UI = {};
    ...
    const logDebug = (message) => {
        ...
        console.log(`[OCR v21.5.20] ${logEntry}`);
    ...
    function activateScanner() {
        logDebug("Activating scanner v21.5.20...");
    ...
    function displayOcrResults(targetImg) {
        ...
        const show = () => { clearTimeout(state.hideTimeout); state.hideTimeout = null; state.overlay.classList.remove('is-hidden'); state.overlay.classList.add('is-focused'); };
        const hide = () => { state.hideTimeout = setTimeout(() => { state.overlay.classList.add('is-hidden'); state.overlay.classList.remove('is-focused'); }, 50); };
        [targetImg, ...overlay.querySelectorAll('.gemini-ocr-text-box')].forEach(el => {
    ...
    /**
     * Recalculates the font size for all text boxes within a specific overlay.
     * This is an expensive function and should only be called when the overlay's dimensions change.
     * It uses a binary search and a shared measurement element for optimal performance.
     */
    function calculateAndApplyFontSizes(overlay) {
        ...
        overlay.querySelectorAll('.gemini-ocr-text-box').forEach(box => {
            ...
            const boxRect = box.getBoundingClientRect(); // Read once
            const availableWidth = boxRect.width - 8; // Inner space for text
            ...
            // Pre-style the shared measurement span once
    ...
    /**
     * The main update loop. Runs on every animation frame.
     * It is optimized to only perform cheap position updates on every frame.
     * The expensive font size calculation is only triggered when an image's dimensions change.
     */
    function updateAllOverlays() {
        ...
        try {
            ...
            for (const [img, state] of managedElements.entries()) {
                ...
                // PHASE 1: Performant Position Update (runs every frame)
                Object.assign(state.overlay.style, {
                    top: `${rect.top + window.scrollY}px`, left: `${rect.left + window.scrollX}px`,
                    width: `${rect.width}px`, height: `${rect.height}px`
                });

                // PHASE 2: Expensive Font Recalculation (runs only when needed)
    ...
    // --- UI & EVENT HANDLING ---
    /**
     * NEW: Periodically checks the URL to apply or remove a scroll fix.
     * This prevents the main scrollbar from being hidden on non-manga pages.
     */
    ...
    function createUI() {
        GM_addStyle(`
            ...
            .gemini-ocr-decoupled-overlay { position: absolute; z-index: 9999; pointer-events: none !important; transition: opacity 0.15s, visibility 0.15s; }
            ...
            .interaction-mode-hover.is-focused .gemini-ocr-text-box:hover { transform: scale(1.05); background: var(--ocr-highlight-bg-color); border-color: var(--ocr-highlight-border-color); color: var(--ocr-highlight-text-color); text-shadow: none; box-shadow: var(--ocr-highlight-shadow); z-index: 10000; }
            ...
            .interaction-mode-click.is-focused .manual-highlight { transform: scale(1.05); background: var(--ocr-highlight-bg-color); border-color: var(--ocr-highlight-border-color); color: var(--ocr-highlight-text-color); text-shadow: none; box-shadow: var(--ocr-highlight-shadow), var(--ocr-highlight-inset-shadow); z-index: 10000; }
        ...
        document.body.insertAdjacentHTML('beforeend', `
            ...
            <div id="gemini-ocr-settings-modal" class="gemini-ocr-modal is-hidden">
                <div class="gemini-ocr-modal-header"><h2>Local OCR Settings (v21.5.20)</h2></div>
    ...
    async function init() {
        ...
        if (loadedSettings) {
            try {
                ...
                // Clean up obsolete settings from previous versions
        ...
        // Start the periodic check for the scroll fix.
        setInterval(manageScrollFix, 500);

        activateScanner();
        if (!overlayUpdateRunning) requestAnimationFrame(updateAllOverlays);
    }
    init().catch(e => console.error(`[OCR v21.5.20] Fatal Initialization Error: ${e.message}`));
})();

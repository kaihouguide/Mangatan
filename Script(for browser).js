// ==UserScript==
// @name         Automatic Content OCR (v21.6.42 - Credential Forwarding)
...
(function() {
    ...
    function processImage(img) {
        ...
        GM_xmlhttpRequest({
            method: 'GET',
            url: ocrRequestUrl, // Use the new URL that may include credentials
            timeout: 30000,
            onload: (res) => {
                try {
    ...
    function createUI() {
        GM_addStyle(`
            ...
            .gemini-ocr-text-box { position: absolute; display: grid; place-items: center; text-align: center; box-sizing: border-box; border-radius: 4px; user-select: text; cursor: pointer; background: var(--ocr-bg-color); border: 2px solid var(--ocr-border-color); color: var(--ocr-text-color); text-shadow: 1px 1px 2px rgba(0,0,0,0.8); backdrop-filter: blur(2px); transition: all 0.2s ease-in-out; pointer-events: auto !important; overflow: hidden; padding: 4px; }
    ...
    function bindUIEvents() {
        ...
        UI.colorThemeSelect.addEventListener('change', () => { applyColorTheme(); document.documentElement.style.setProperty('--modal-header-color', COLOR_THEMES[UI.colorThemeSelect.value].main + '1)'); });

// ==UserScript==
// @name         Automatic Content OCR (Mobile - Long-Press Toggle v7.4)
...
(function() {
    ...
    const LONG_PRESS_DURATION = 500; // Time in ms to detect a long press
    ...
    function activateScanner() {
        logDebug("Activating passive scanner v7.4 (Long-Press FINAL)...");
    ...
    // --- DISPLAY LOGIC (Managed Phantom Overlay with Long-Press) ---
    function displayOcrResults(targetImg) {
        ...
        const onHold = () => {
            overlayContainer.classList.toggle('is-visible-ocr');
            logDebug(`Overlay toggled via long-press for ...${targetImg.src.slice(-20)}`);
        ...
        // Assign events for both mouse and touch
        targetImg.addEventListener("mousedown", pressDown);
        targetImg.addEventListener("touchstart", pressDown, { passive: true });

        targetImg.addEventListener("mouseup", pressUp);
        targetImg.addEventListener("mouseleave", pressUp);
        ...
        // Capture the click event *after* a press and prevent it only if our long-press was triggered.
    ...
    async function createUI() {
        GM_addStyle(`
            ...
            @media (max-width: 600px) {
                ...
                .gemini-ocr-settings-grid > label:not([class="full-width"]) { margin-left: 0; }
                .gemini-ocr-modal-buttons { justify-content: space-between; }
                .gemini-ocr-modal input, .gemini-ocr-modal select, .gemini-ocr-modal textarea { width: 100% !important; }
            }
        `);
        ...
        modal.innerHTML = `
            <h2 class="full-width">Local OCR Settings</h2>
            <div class="gemini-ocr-settings-grid">
                <h3 class="full-width">Connection</h3>
                <label class="full-width" for="gemini-ocr-server-url">OCR Server URL: <em style="font-weight:normal;color:#ccc;">(On mobile, use your PC's local IP, e.g., http://192.168.1.10:3000)</em></label>
                ...
                <div class="gemini-ocr-modal-buttons full-width"> <button id="gemini-ocr-debug-btn" style="background-color: #777; margin-right: auto;">Logs</button> <button id="gemini-ocr-close-btn" style="background-color: #555;">Close</button> <button id="gemini-ocr-save-btn">Save</button> </div>
            </div>`;
        ...
        const loadedSettings = await GM_getValue('gemini_ocr_observer_settings_v7.4');
        ...
        document.getElementById('gemini-ocr-save-btn').addEventListener('click', async () => {
            ...
            await GM_setValue('gemini_ocr_observer_settings_v7.4', JSON.stringify(newSettings));
    ...
    function checkServerStatus() {
        ...
        GM_xmlhttpRequest({
            method: 'GET', url: settings.ocrServerUrl, timeout: 5000,
            onload: (res) => {
                try { const data = JSON.parse(res.responseText); if (data.status === 'running') { statusDiv.className = 'status-ok'; statusDiv.textContent = `Connected (${data.items_in_cache} cached)`; } else { throw new Error('Invalid response'); } } catch (e) { statusDiv.className = 'status-error'; statusDiv.textContent = 'Unresponsive Server'; }
            },

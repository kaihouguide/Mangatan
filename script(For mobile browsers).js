...
(function() {
    ...
    let debugLog = [];
    const SETTINGS_KEY = 'gemini_ocr_settings_v25_mobile_pc_port';
    ...
    function observeImageForSrcChange(img) {
        const processTheImage = (src) => {
            if (src?.includes('/api/v1/manga/')) {
                ...
                if (img.complete && img.naturalHeight > 0) { primeImageForOcr(img); }
                else { img.addEventListener('load', () => primeImageForOcr(img), { once: true }); }
                return true;
            }
            return false;
        };
    ...
    function calculateAndApplyFontSizes(overlay, imgRect) {
        ...
        textBoxes.forEach(box => {
            ...
            let bestSize = 8, multiplier; measurementSpan.textContent = text;
            if (box.classList.contains('gemini-ocr-text-vertical')) {
                ...
                while (low <= high) {
                    ...
                    if ((measurementSpan.offsetWidth <= availableHeight) && (measurementSpan.offsetHeight <= availableWidth)) { bestSize = mid; low = mid + 1; } else { high = mid - 1; }
                }
    ...
    async function exportImageToAnki(targetImg) {
        logDebug(`Anki Export: Starting screenshot...`); if (!settings.ankiImageField) { alert('Anki Image Field is not set in settings.'); return false; } if (!targetImg || !targetImg.complete || !targetImg.naturalHeight) { alert('Anki Export Failed: The selected image is not valid.'); return false; }
        try {
    ...
    function manageScrollFix() { const href = window.location.href; if (href.includes('/manga/') || href.includes('/chapter/')) { if (!document.documentElement.classList.contains('ocr-scroll-fix-active')) document.documentElement.classList.add('ocr-scroll-fix-active'); } else { if (document.documentElement.classList.contains('ocr-scroll-fix-active')) document.documentElement.classList.remove('ocr-scroll-fix-active'); } }
    ...
    function createUI() {
        ...
        document.body.insertAdjacentHTML('beforeend', `
            ...
            <div id="gemini-ocr-settings-modal" class="gemini-ocr-modal is-hidden">
                <div class="gemini-ocr-modal-header"><h2>OCR Mobile Settings</h2></div>
                <div class="gemini-ocr-modal-content">
                    <h3>Connection & Image Source</h3><div class="gemini-ocr-settings-grid full-width">
                    ...
                    <h3>Display & Readability</h3><div class="gemini-ocr-settings-grid">
    ...
    function bindUIEvents() {
        ...
        UI.saveBtn.addEventListener('click', async () => {
            const newSettings = {
                ...
                textOrientation: UI.textOrientationSelect.value, colorTheme: UI.colorThemeSelect.value,
    ...
    init().catch(e => console.error(`[OCR Mobile] Fatal Initialization Error: ${e.message}`));
})();

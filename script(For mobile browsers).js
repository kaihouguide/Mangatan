// ==UserScript==
// @name         Automatic Content OCR (v21.6.46 - Mobile Port)
// @namespace    http://tampermonkey.net/
// @version      21.6.46
// @description  Passes image source credentials through the OCR server and adds proximity mode, opacity control, and other new features. Optimized for mobile.
// @author       1Selxo (Ported and Merged by Gemini)
// @match        *://127.0.0.1*/*
// @grant        GM_setValue
// @grant        GM_getValue
// @grant        GM_addStyle
...
(function() {
    ...
    let settings = {
        ...
        imageServerUser: '',      // Credentials for the IMAGE SOURCE (e.g., Suwayomi)
        imageServerPassword: '',  //
        ...
        interactionMode: 'click', // Default for mobile
        proximityRadius: 150,     // New feature from v21.6.46
        dimmedOpacity: 0.3,       // New feature from v21.6.46
        colorTheme: 'deepblue',
        fontMultiplierHorizontal: 1.0,
        fontMultiplierVertical: 1.0
    };
    let debugLog = [];
    const SETTINGS_KEY = 'gemini_ocr_settings_v21_6_mobile_credential_forward_merged'; // Updated key
    ...
    const logDebug = (message) => {
        ...
        console.log(`[OCR v21.6.46 Mobile] ${logEntry}`);
    ...
    // --- Image Processing (No-Cache Workflow) ---
    function observeImageForSrcChange(img) {
        const processTheImage = (src) => {
            // Updated condition to be more specific, from PC version
    ...
    function displayOcrResults(targetImg) {
        ...
        data.sort((a, b) => {
            ...
            const a_x = a.tightBoundingBox.x; const b_x = b.tightBoundingBox.x;
        ...
        data.forEach((item) => {
            ...
            // Improved centering from PC version
        ...
        // --- Mobile Interaction Logic (Merged) ---
        const show = (e) => {
            e.preventDefault();
        ...
        if (settings.interactionMode === 'click') {
            ...
            overlay.addEventListener('touchmove', (e) => {
                ...
                frameRequest = requestAnimationFrame(() => {
                    ...
                    const touchX = touch.clientX - overlayRect.left;
                    const touchY = touch.clientY - overlayRect.top;
                    textBoxes.forEach(box => {
                        const boxCenterX = box.offsetLeft + box.offsetWidth / 2;
                        const boxCenterY = box.offsetTop + box.offsetHeight / 2;
                        if (Math.hypot(boxCenterX - touchX, boxCenterY - touchY) < settings.proximityRadius) {
             ...
             overlay.addEventListener('touchend', () => {
                 ...
                 textBoxes.forEach(box => box.classList.remove('is-near'));
             });
        }
        if (!overlayUpdateRunning) requestAnimationFrame(updateAllOverlays);
    }

    // --- FONT CALCULATION (UNIFIED PRECISE ENGINE) ---
    ...
    async function ankiConnectRequest(action, params = {}) {
        ...
        return new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                ...
                onerror: () => reject(new Error('Connection to Anki-Connect failed. Is Anki running?')),
    ...
    async function exportImageToAnki(targetImg) {
        ...
        try {
            ...
            if (error.message.includes("SecurityError") || error.message.includes("tainted")) { alert(`Anki Export Failed: Canvas security error due to CORS policy.`); }
    ...
    // --- applyStyles (Merged from PC version) ---
    ...
    function createUI() {
        GM_addStyle(`
            ...
            /* MERGED: Highlighted Box State (applies to all modes) */
            ...
            .interaction-mode-proximity.is-focused .is-near {
                overflow: visible; transform: scale(1.05); background: var(--ocr-highlight-bg-color); border-color: var(--ocr-highlight-border-color); color: var(--ocr-highlight-text-color); text-shadow: none; box-shadow: var(--ocr-highlight-shadow), var(--ocr-highlight-inset-shadow); z-index: 9999; opacity: 1;
            }
            /* MERGED: Dimmed Box State (applies to all modes) */
            ...
            <div id="gemini-ocr-settings-modal" class="gemini-ocr-modal is-hidden">
                ...
                <div class="gemini-ocr-modal-content">
                    <h3>OCR & Image Source</h3><div class="gemini-ocr-settings-grid full-width">
                        ...
                        <label for="gemini-image-server-user">Image Source Username:</label><input type="text" id="gemini-image-server-user" autocomplete="username" placeholder="e.g., Suwayomi Username (Optional)">
                        <label for="gemini-image-server-password">Image Source Password:</label><input type="password" id="gemini-image-server-password" autocomplete="current-password" placeholder="e.g., Suwayomi Password">
                    </div>
                    ...
                    <h3>Interaction & Display</h3><div class="gemini-ocr-settings-grid">
                        ...
                        <label for="ocr-interaction-mode">Highlight Mode:</label><select id="ocr-interaction-mode"><option value="hover">On Hover (PC)</option><option value="click">On Click</option><option value="proximity">On Proximity</option></select>
    ...
    function bindUIEvents() {
        // Full, readable document.getElementById from PC bugfix
        Object.assign(UI, {
        ...
        UI.colorThemeSelect.addEventListener('change', () => { applyStyles(); document.documentElement.style.setProperty('--modal-header-color', COLOR_THEMES[UI.colorThemeSelect.value].main + '1)'); });
        UI.interactionModeSelect.addEventListener('change', () => {
            // New logic to show/hide proximity radius setting
            const isProximity = UI.interactionModeSelect.value === 'proximity';
            const radiusLabel = UI.proximityRadiusInput.previousElementSibling;
            if (radiusLabel && radiusLabel.parentElement) {
                 radiusLabel.parentElement.style.display = isProximity ? '' : 'none';
            }
        });
    ...
    async function init() {
        ...
        createUI();
        bindUIEvents();
        applyStyles(); // Use the new applyStyles function
        createMeasurementSpan();

        // Populate UI with loaded/default settings

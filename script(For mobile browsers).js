// ==UserScript==
// @name         Automatic Content OCR (v21.6.47 - Mobile Hotfix)
// @namespace    http://tampermonkey.net/
// @version      21.6.47
// @description  Fixes scrolling outside of chapter view and improves settings panel layout and functionality for mobile devices.
// @author       1Selxo (Ported and Patched by Gemini)
// @match        *://127.0.0.1*/*
// @grant        GM_setValue
// @grant        GM_getValue
// @grant        GM_addStyle
...
(function() {
    ...
    let debugLog = [];
    const SETTINGS_KEY = 'gemini_ocr_settings_v21_6_mobile_hotfix'; // Updated key
    ...
    function displayOcrResults(targetImg) {
        ...
        const show = (e) => {
            e.preventDefault(); clearTimeout(hideButtonTimer); clearTimeout(state.hideTimeout);
            overlay.classList.remove('is-hidden'); overlay.classList.add('is-focused');
            UI.globalAnkiButton?.classList.remove('is-hidden'); activeImageForExport = targetImg;
        };
        const hide = () => {
        ...
        if (settings.interactionMode === 'click') {
            ...
            overlay.addEventListener('touchmove', (e) => {
                if (frameRequest) return; const touch = e.touches[0]; if (!touch) return;
                frameRequest = requestAnimationFrame(() => {
                    const overlayRect = overlay.getBoundingClientRect(), touchX = touch.clientX - overlayRect.left, touchY = touch.clientY - overlayRect.top;
                    textBoxes.forEach(box => {
                        const boxCenterX = box.offsetLeft + box.offsetWidth / 2, boxCenterY = box.offsetTop + box.offsetHeight / 2;
                        if (Math.hypot(boxCenterX - touchX, boxCenterY - touchY) < settings.proximityRadius) box.classList.add('is-near');
    ...
    async function exportImageToAnki(targetImg) {
        logDebug(`Anki Export: Starting screenshot...`); if (!settings.ankiImageField) { alert('Anki Image Field is not set in settings.'); return false; } if (!targetImg || !targetImg.complete || !targetImg.naturalHeight) { alert('Anki Export Failed: Image not valid.'); return false; }
        try {
            ...
            const filename = `screenshot_${Date.now()}.png`; await ankiConnectRequest('storeMediaFile', { filename, data: base64data }); logDebug(`Anki Export: Stored as '${filename}'`);
            const notes = await ankiConnectRequest('findNotes', { query: 'added:1' }); if (!notes || notes.length === 0) throw new Error('No recently added cards found.'); const lastNoteId = notes.sort((a, b) => b - a)[0]; logDebug(`Anki Export: Found last card ID ${lastNoteId}`);
    ...
    // --- HOTFIX: More specific scroll management ---
    function manageScrollFix() {
        // Only apply on chapter reader pages to avoid breaking scroll elsewhere.
        const shouldBeActive = window.location.href.includes('/manga/') && window.location.href.includes('/chapter/');
    ...
    function createUI() {
        GM_addStyle(`
            ...
            .gemini-ocr-text-box { position: absolute; display: flex; justify-content: center; align-items: center; text-align: center; box-sizing: border-box; border-radius: 4px; user-select: text; cursor: pointer; background: var(--ocr-bg-color); border: 2px solid var(--ocr-border-color); color: var(--ocr-text-color); text-shadow: 1px 1px 2px rgba(0,0,0,0.8); backdrop-filter: blur(2px); transition: all 0.2s ease-in-out; pointer-events: auto !important; overflow: hidden; padding: 4px; }
            ...
            .interaction-mode-hover.is-focused:has(.gemini-ocr-text-box:hover) .gemini-ocr-text-box:not(:hover), .interaction-mode-click.is-focused.has-manual-highlight .gemini-ocr-text-box:not(.manual-highlight), .interaction-mode-proximity.is-focused .gemini-ocr-text-box:not(.is-near) { opacity: var(--ocr-dimmed-opacity); background: rgba(10,25,40,0.5); border-color: var(--ocr-border-color-dim); }
            ...
            #gemini-ocr-global-anki-export-btn:disabled { background-color: #95a5a6; cursor: wait; transform: none; } #gemini-ocr-global-anki-export-btn.is-hidden { opacity: 0; visibility: hidden; pointer-events: none; transform: scale(0.5); }
            .gemini-ocr-modal { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background-color: #1A1D21; border: 1px solid var(--modal-header-color); z-index: 2147483647; color: #EAEAEA; font-family: sans-serif; box-shadow: 0 8px 32px 0 rgba(0,0,0,0.5); display: flex; flex-direction: column; max-width: 100vw; max-height: 100vh; border-radius: 0; }
            .gemini-ocr-modal.is-hidden { display: none; } .gemini-ocr-modal-header { padding: 15px 20px; border-bottom: 1px solid #444; } .gemini-ocr-modal-header h2 { margin: 0; color: var(--modal-header-color); font-size: 1.2em; }
            .gemini-ocr-modal-content { padding: 10px 20px; overflow-y: auto; flex-grow: 1; -webkit-overflow-scrolling: touch; } .gemini-ocr-modal-footer { padding: 10px 20px; border-top: 1px solid #444; display: flex; justify-content: flex-end; gap: 10px; align-items: center; flex-wrap: wrap;}
            .gemini-ocr-modal h3 { font-size: 1.1em; margin: 15px 0 10px 0; border-bottom: 1px solid #333; padding-bottom: 8px; color: var(--modal-header-color); }
            /* HOTFIX: More compact 2-column grid for settings, better for mobile */
            ...
            .gemini-ocr-modal label { white-space: nowrap; }
            .gemini-ocr-modal input, .gemini-ocr-modal textarea, .gemini-ocr-modal select { width: 100%; padding: 10px; box-sizing: border-box; font-size: 14px; background-color: #2a2a2e; border: 1px solid #555; border-radius: 5px; color: #EAEAEA; }
            .gemini-ocr-modal button { padding: 10px 18px; border-radius: 5px; cursor: pointer; font-weight: bold; font-size: 14px; }
            ...
            <div id="gemini-ocr-settings-modal" class="gemini-ocr-modal is-hidden">
                ...
                <div class="gemini-ocr-modal-content">
                    <h3>OCR & Image Source</h3><div class="gemini-ocr-settings-grid full-width">
                        ...
                        <label for="gemini-image-server-user">Image Source User:</label><input type="text" id="gemini-image-server-user" autocomplete="username" placeholder="Optional">
                        <label for="gemini-image-server-password">Image Source Pass:</label><input type="password" id="gemini-image-server-password" autocomplete="current-password" placeholder="Optional">
                    </div>
                    <div id="gemini-ocr-server-status" class="full-width" style="margin-top: 10px;">Click to check status</div>
                    <h3>Anki Integration</h3><div class="gemini-ocr-settings-grid">
                        ...
                        <label for="gemini-ocr-anki-field">Image Field Name:</label><input type="text" id="gemini-ocr-anki-field">
                    </div>
                    <h3>Interaction & Display</h3><div class="gemini-ocr-settings-grid">
                        ...
                        <label for="ocr-interaction-mode">Highlight Mode:</label><select id="ocr-interaction-mode"><option value="click">On Click</option><option value="proximity">On Proximity</option><option value="hover">On Hover (PC)</option></select>
                        <label for="ocr-dimmed-opacity">Dim Opacity (%):</label><input type="number" id="ocr-dimmed-opacity" min="0" max="100" step="5">
                        <label for="ocr-proximity-radius">Proximity Radius:</label><input type="number" id="ocr-proximity-radius" min="10" max="1000" step="10">
                        ...
                        <label for="ocr-font-multiplier-horizontal">H. Font Size:</label><input type="number" id="ocr-font-multiplier-horizontal" min="0.1" max="5" step="0.1">
                        <label for="ocr-font-multiplier-vertical">V. Font Size:</label><input type="number" id="ocr-font-multiplier-vertical" min="0.1" max="5" step="0.1">
                    </div>
                    <h3>Advanced</h3><div class="gemini-ocr-settings-grid"><label for="gemini-ocr-debug-mode">Debug Mode</label><input type="checkbox" id="gemini-ocr-debug-mode" style="width: auto; justify-self: start;"></div>
                    <div class="full-width"><label for="gemini-ocr-sites-config">Site Configurations:</label><textarea id="gemini-ocr-sites-config" rows="4" placeholder="URL; OverflowFix; .container1; ..."></textarea></div>
                </div>
                <div class="gemini-ocr-modal-footer"><button id="gemini-ocr-debug-btn" style="background-color: #777; margin-right: auto;">Logs</button><button id="gemini-ocr-close-btn" style="background-color: #555;">Close</button><button id="gemini-ocr-save-btn" style="background-color: var(--modal-header-color);">Save & Reload</button></div>
            </div>
    ...
    function bindUIEvents() {
        ...
        UI.globalAnkiButton.addEventListener('click', async () => { if (!activeImageForExport) { alert("Tap on an image to select it for export."); return; } const btn = UI.globalAnkiButton; btn.textContent = '…'; btn.disabled = true; const success = await exportImageToAnki(activeImageForExport); if (success) { btn.textContent = '✓'; btn.style.backgroundColor = '#27ae60'; } else { btn.textContent = '✖'; btn.style.backgroundColor = '#c0392b'; } setTimeout(() => { btn.textContent = '✚'; btn.style.backgroundColor = ''; btn.disabled = false; }, 2000); });
        ...
        UI.colorThemeSelect.addEventListener('change', () => { settings.colorTheme = UI.colorThemeSelect.value; applyStyles(); document.documentElement.style.setProperty('--modal-header-color', COLOR_THEMES[settings.colorTheme].main + '1)'); });

        // --- HOTFIX: Robustly show/hide proximity setting ---
        UI.interactionModeSelect.addEventListener('change', () => {
            const isProximity = UI.interactionModeSelect.value === 'proximity';
            const radiusInput = UI.proximityRadiusInput;
            const radiusLabel = document.querySelector('label[for="ocr-proximity-radius"]');
            const displayStyle = isProximity ? '' : 'none';
            if(radiusInput) radiusInput.style.display = displayStyle;
            if(radiusLabel) radiusLabel.style.display = displayStyle;
        });
        ...
        UI.saveBtn.addEventListener('click', async () => {
            const newSettings = {
                ...
                proximityRadius: parseInt(UI.proximityRadiusInput.value, 10) || 150, dimmedOpacity: (parseInt(UI.dimmedOpacityInput.value, 10) || 30) / 100,
    ...
    async function init() {
        ...
        if (loadedSettings) { try { const parsed = JSON.parse(loadedSettings); settings = { ...settings, ...parsed }; } catch(e) { logDebug("Could not parse settings. Using defaults."); } }
        ...
        UI.dimmedOpacityInput.value = settings.dimmedOpacity * 100; UI.proximityRadiusInput.value = settings.proximityRadius;
        ...
        // Trigger change event to correctly show/hide proximity settings on load

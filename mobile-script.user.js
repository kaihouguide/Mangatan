...
(function() {
    ...
    // --- Rendering Logic (Synced with PC) ---
    ...
    function bindUIEvents() {
        ...
        UI.globalEditButton.addEventListener('click', (e) => { e.stopPropagation(); if (activeOverlay) { activeOverlay.classList.toggle('edit-mode-active'); UI.globalEditButton.classList.toggle('edit-active'); if (!activeOverlay.classList.contains('edit-mode-active')) { activeMergeSelections.delete(activeOverlay); activeOverlay.querySelectorAll('.selected-for-merge').forEach(el => el.classList.remove('selected-for-merge')); } else { updateEditActionBar(activeOverlay); } } });
    ...
    async function init() {
        ...
        logDebug("Initializing MOBILE HYBRID engine (Server-Synced).");

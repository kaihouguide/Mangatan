...
// @version      24.5.25-M-BlendModeFix
// @description  A mobile-optimized OCR overlay. Replaces flawed inverse color with a superior CSS blend mode for perfect text contrast on any background.
...
(function() {
    ...
    let settings = {
        ...
        focusFontColor: 'default' // --- MODIFIED --- Can be 'default', 'black', 'white', or 'difference'
    };
    let debugLog = [];
    ...
    // --- MODIFIED --- Simplified logic to use CSS classes for focus colors.
    function handleOverlayInteraction(event) {
        ...
        if (overlay.classList.contains('edit-mode-active')) {
            ...
            if (!clickedBox.classList.contains('manual-highlight')) {
                // Clear previous highlight and its focus color class
                ...
                // Apply the correct focus color class based on settings
    ...
    function createUI() {
        GM_addStyle(`
            ...
            .gemini-ocr-decoupled-overlay.is-focused { display: block; opacity: 1; pointer-events: auto; background-color: transparent; }
            ...
            .gemini-ocr-text-box { position: absolute; display: flex; align-items: center; justify-content: center; text-align: center; box-sizing: border-box; user-select: text; cursor: pointer; transition: all 0.2s ease-in-out; overflow: hidden; font-family: 'Noto Sans JP', sans-serif; font-weight: 600; padding: 4px; border-radius: 4px; border: none; text-shadow: none; background: transparent; }
            body.ocr-brightness-light .gemini-ocr-text-box { color: rgba(var(--accent), 0.5); }
            body.ocr-brightness-dark .gemini-ocr-text-box { color: rgba(var(--background), 0.7); }
            .is-focused .gemini-ocr-text-box.manual-highlight, .is-focused .gemini-ocr-text-box.selected-for-merge { background: rgba(var(--background), 1); color: rgba(var(--accent), 1); box-shadow: 0 0 0 0.1em rgba(var(--background), 1), 0 0 0 0.2em rgba(var(--accent), 1); }
            body.ocr-brightness-dark .gemini-ocr-text-box { background: rgba(29, 34, 39, 0.9); }
            ...
            /* --- NEW --- Focus color logic using classes */
            .is-focused .gemini-ocr-text-box.manual-highlight.ocr-focus-color-black { color: #000000 !important; text-shadow: 0 0 2px #FFFFFF, 0 0 4px #FFFFFF; }
            .is-focused .gemini-ocr-text-box.manual-highlight.ocr-focus-color-white { color: #FFFFFF !important; text-shadow: 0 0 2px #000000, 0 0 4px #000000; }
            .is-focused .gemini-ocr-text-box.manual-highlight.ocr-focus-color-difference { color: white !important; mix-blend-mode: difference; background: transparent !important; box-shadow: none !important; }
        ...
        `);
        // --- MODIFIED --- Updated the settings modal HTML
    ...
    async function init() {
        ...
        logDebug("Initializing MOBILE HYBRID engine (BlendMode Fix).");

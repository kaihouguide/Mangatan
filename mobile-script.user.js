...
// @version      24.4.6-M
// @description  Adds a mobile-optimized OCR overlay with an intuitive touch-based editor for merging and deleting text. Uses a high-performance hybrid rendering model for perfectly smooth scrolling.
// @author       1Selxo (Mobile port by Gemini, Hybrid Rendering & Editor by Gemini)
// @match        *://127.0.0.1*/*
// @grant        GM_setValue
// @grant        GM_getValue
// @grant        GM_addStyle
...
(function() {
    ...
    let settings = {
        ...
        boundingBoxAdjustment: 5, // Ported from PC
        focusScaleMultiplier: 1.2,
        soloHoverMode: false, // Ported from PC
        colorTheme: 'deepblue'
    };
    let debugLog = [];
    const SETTINGS_KEY = 'gemini_ocr_settings_v24_M_editor'; // Updated key
    ...
    function handleGlobalTap(event) {
        ...
        if (!activeOverlay.contains(target) && !target.closest('#gemini-ocr-settings-button, #gemini-ocr-global-anki-export-btn')) hideActiveOverlay();
    }
    ...
    // --- Mobile Editor Logic (Ported from PC) ---
    ...
    function handleMergeSelection(boxElement, sourceImage, overlay) {
        ...
        if (!currentSelection) {
            ...
            if (firstBox === secondBox) return; // Can't merge with self
    ...
    function handleOverlayInteraction(event) {
        ...
        const sourceImage = activeImageForExport; // Assuming this is set when overlay is shown
    ...
    function displayOcrResults(targetImg) {
        ...
        // Add Mobile Editor Controls
        const controlsContainer = document.createElement('div');
        controlsContainer.className = 'gemini-ocr-overlay-controls';
        const editButton = document.createElement('button');
        editButton.className = 'gemini-ocr-edit-btn'; editButton.textContent = 'Edit';
        const deleteButton = document.createElement('button');
        deleteButton.className = 'gemini-ocr-delete-btn'; deleteButton.textContent = 'Delete Selected';
        
        controlsContainer.append(editButton, deleteButton);
        overlay.appendChild(controlsContainer);
        
        editButton.addEventListener('click', (e) => {
            e.stopPropagation();
            overlay.classList.toggle('edit-mode-active');
            activeMergeSelections.delete(overlay); // Clear selection on mode toggle

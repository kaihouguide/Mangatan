...
// @version      24.5.21-M-Stable
// @description  A mobile-optimized OCR overlay synced with the PC version. Features multi-merge, a dedicated edit mode, robust navigation detection, and improved interaction.
...
(function() {
    ...
    let debugLog = [];
    const SETTINGS_KEY = 'gemini_ocr_settings_v24_m_synced';
    ...
    function fullCleanupAndReset() {
        ...
        managedElements.clear(); managedContainers.clear(); visibleImages.clear(); activeMergeSelections.clear();
    ...
    function setupNavigationObserver() {
        ...
        if (!contentRootSelector) { logDebug("Warning: No `contentRootSelector` defined."); return; }
        const targetNode = document.querySelector(contentRootSelector);
        if (!targetNode) { logDebug(`Navigation observer could not find target node: ${contentRootSelector}.`); return; }
    ...
    const handleResize = (entries) => {
        for (const entry of entries) {
            const img = entry.target; const state = managedElements.get(img);
        ...
        for (const entry of entries) {
            ...
            if (entry.isIntersecting) {
                if (!visibleImages.has(img)) {
                    visibleImages.add(img); const state = managedElements.get(img);
                ...
                if (visibleImages.has(img)) {
                    visibleImages.delete(img); const state = managedElements.get(img);
                    if (state) state.overlay.style.visibility = 'hidden';
                    if (visibleImages.size === 0 && animationFrameId !== null) { cancelAnimationFrame(animationFrameId); animationFrameId = null; }
                }
            }
        }
    };
    ...
    function observeImageForSrcChange(img) {
        ...
        const attributeObserver = new MutationObserver((mutations) => {
            for (const mutation of mutations) if (mutation.attributeName === 'src' && processTheImage(img.src)) { attributeObserver.disconnect(); attachedAttributeObservers.delete(img); break; }
        });
    ...
    function primeImageForOcr(img) {
        ...
        const process = () => { if (managedElements.has(img) || ocrDataCache.get(img) === 'pending') return; img.crossOrigin = "anonymous"; processImage(img, img.src); };
        if (img.complete && img.naturalHeight > 0) process(); else img.addEventListener('load', process, { once: true });
    }
    ...
    function calculateAndApplyStylesForSingleBox(box, imgRect) {
        ...
        const ocrData = box._ocrData; const text = ocrData.text || '';
        const availableWidth = box.offsetWidth + settings.boundingBoxAdjustment; const availableHeight = box.offsetHeight + settings.boundingBoxAdjustment;
        if (!text || availableWidth <= 0 || availableHeight <= 0) return;
        const isMerged = ocrData?.isMerged || text.includes('\u200B'); const isMergedVertical = ocrData?.forcedOrientation === 'vertical';
        const findBestFitSize = (isVerticalSearch) => {
            measurementSpan.style.writingMode = isVerticalSearch ? 'vertical-rl' : 'horizontal-tb';
            if (isMerged) { measurementSpan.innerHTML = box.innerHTML; measurementSpan.style.whiteSpace = 'normal'; } else { measurementSpan.textContent = text; measurementSpan.style.whiteSpace = 'nowrap'; }
            let low = 1, high = 200, bestSize = 1;
            while (low <= high) {
                const mid = Math.floor((low + high) / 2); if (mid <= 0) break;
                measurementSpan.style.fontSize = `${mid}px`;
                let textFits = (isMerged) ? (measurementSpan.offsetWidth <= availableWidth && measurementSpan.offsetHeight <= availableHeight) : (isVerticalSearch ? measurementSpan.offsetHeight <= availableHeight : measurementSpan.offsetWidth <= availableWidth);
                if (textFits) { bestSize = mid; low = mid + 1; } else { high = mid - 1; }
            }
            measurementSpan.style.whiteSpace = ''; measurementSpan.style.writingMode = ''; measurementSpan.innerHTML = ''; return bestSize;
        };
        const horizontalFitSize = findBestFitSize(false); const verticalFitSize = findBestFitSize(true);
        ...
        else { if (verticalFitSize > horizontalFitSize) { isVertical = true; finalFontSize = verticalFitSize; } else { isVertical = false; finalFontSize = horizontalFitSize; } }
    ...
    function calculateAndApplyOptimalStyles_Optimized(overlay, imgRect) {
        ...
        const boxes = Array.from(overlay.querySelectorAll('.gemini-ocr-text-box')); if (boxes.length === 0) return;
    ...
    function autoMergeOcrData(lines) {
        ...
        const horizontalLines = lines.filter(l => l.tightBoundingBox.width > l.tightBoundingBox.height); const verticalLines = lines.filter(l => l.tightBoundingBox.width <= l.tightBoundingBox.height);
        ...
        const initialMedianLineHeight = median(horizontalLines.map(l => l.tightBoundingBox.height * 1000)); const initialMedianLineWidth = median(verticalLines.map(l => l.tightBoundingBox.width * 1000));
        const primaryHorizontalLines = horizontalLines.filter(l => (l.tightBoundingBox.height * 1000) >= initialMedianLineHeight * settings.autoMergeMinLineRatio); const primaryVerticalLines = verticalLines.filter(l => (l.tightBoundingBox.width * 1000) >= initialMedianLineWidth * settings.autoMergeMinLineRatio);
        const robustMedianLineHeight = median(primaryHorizontalLines.map(l => l.tightBoundingBox.height * 1000)) || initialMedianLineHeight || 20; const robustMedianLineWidth = median(primaryVerticalLines.map(l => l.tightBoundingBox.width * 1000)) || initialMedianLineWidth || 20;
        const processedLines = lines.map((line, index) => { const bbox = line.tightBoundingBox; const isVertical = bbox.width <= bbox.height; const fontSize = isVertical ? bbox.width * 1000 : bbox.height * 1000; return { ...line, originalIndex: index, isVertical, fontSize, bbox: { x: bbox.x * 1000, y: bbox.y * 1000, width: bbox.width * 1000, height: bbox.height * 1000, right: (bbox.x + bbox.width) * 1000, bottom: (bbox.y + bbox.height) * 1000 } }; });
        const uf = new UnionFind(lines.length);
        for (let i = 0; i < processedLines.length; i++) {
            for (let j = i + 1; j < processedLines.length; j++) {
                const lineA = processedLines[i]; const lineB = processedLines[j]; if (lineA.isVertical !== lineB.isVertical) continue;
                const isLineAPrimary = lineA.fontSize >= (lineA.isVertical ? robustMedianLineWidth : robustMedianLineHeight) * settings.autoMergeMinLineRatio; const isLineBPrimary = lineB.fontSize >= (lineB.isVertical ? robustMedianLineWidth : robustMedianLineHeight) * settings.autoMergeMinLineRatio;
                let currentFontRatioThreshold = settings.autoMergeFontRatio; if ((isLineAPrimary && !isLineBPrimary) || (!isLineAPrimary && isLineBPrimary)) { currentFontRatioThreshold = settings.autoMergeFontRatioForMixed; }
                const fontRatio = Math.max(lineA.fontSize / lineB.fontSize, lineB.fontSize / lineA.fontSize); if (fontRatio > currentFontRatioThreshold) continue;
                ...
                let readingGap, perpOverlap, perpOffset; const smallerPerpSize = Math.min(lineA.isVertical ? lineA.bbox.height : lineA.bbox.width, lineA.isVertical ? lineB.bbox.height : lineB.bbox.width);
                if (lineA.isVertical) { readingGap = Math.max(0, Math.max(lineA.bbox.x, lineB.bbox.x) - Math.min(lineA.bbox.right, lineB.bbox.right)); perpOverlap = Math.max(0, Math.min(lineA.bbox.bottom, lineB.bbox.bottom) - Math.max(lineA.bbox.y, lineB.bbox.y)); perpOffset = Math.abs((lineA.bbox.y + lineA.bbox.height / 2) - (lineB.bbox.y + lineB.bbox.height / 2)); } else { readingGap = Math.max(0, Math.max(lineA.bbox.y, lineB.bbox.y) - Math.min(lineA.bbox.bottom, lineB.bbox.bottom)); perpOverlap = Math.max(0, Math.min(lineA.bbox.right, lineB.bbox.right) - Math.max(lineA.bbox.x, lineB.bbox.x)); perpOffset = Math.abs((lineA.bbox.x + lineA.bbox.width / 2) - (lineB.bbox.x + lineB.bbox.width / 2)); }
                const perpTol = (lineA.isVertical ? settings.autoMergePerpTol * robustMedianLineHeight : settings.autoMergePerpTol * robustMedianLineWidth); if (readingGap > distThreshold) continue; if (perpOffset > perpTol && perpOverlap / smallerPerpSize < settings.autoMergeOverlapMin) continue; if (((isLineAPrimary && !isLineBPrimary) || (!isLineAPrimary && isLineBPrimary)) && (perpOverlap / smallerPerpSize < settings.autoMergeMixedMinOverlapRatio)) { continue; }
                uf.union(i, j);
            }
        }
        const groups = {}; for (let i = 0; i < processedLines.length; i++) { const root = uf.find(i); if (!groups[root]) groups[root] = []; groups[root].push(processedLines[i]); }
        const finalMergedData = [];
        for (const rootId in groups) {
            const group = groups[rootId];
            if (group.length === 1) { finalMergedData.push(lines[group[0].originalIndex]); } else {
                const isVertical = group[0].isVertical;
                group.sort((a, b) => { if (isVertical) { return (Math.abs(b.bbox.x - a.bbox.x) > robustMedianLineWidth / 2) ? b.bbox.x - a.bbox.x : a.bbox.y - b.bbox.y; } else { return (Math.abs(a.bbox.y - b.bbox.y) > robustMedianLineHeight / 2) ? a.bbox.y - b.bbox.y : a.bbox.x - b.bbox.x; } });
    ...
    // --- [NEW] Mobile Touch, Interaction & Multi-Merge/Delete Logic ---
    ...
    function handleTouchStart(event) {
        ...
        if (settings.activationMode === 'doubleTap') {
            const now = Date.now(); const lastTap = tapTracker.get(targetImg);
    ...
    function handleTouchMove(event) {
        if (!longPressState.valid) return; const SCROLL_TOLERANCE = 10;
        const deltaX = Math.abs(longPressState.startX - event.touches[0].clientX); const deltaY = Math.abs(longPressState.startY - event.touches[0].clientY);
    ...
    function handleContextMenu(event) {
        if (settings.activationMode === 'longPress' && longPressState.valid && event.target === longPressState.target) {
            ...
            triggerOverlayToggle(longPressState.target); longPressState.valid = false;
            return false;
        }
    }
    function handleGlobalTap(event) {
        if (!activeOverlay) return; const target = event.target;
        if (!target.closest('.gemini-ocr-text-box, .gemini-ocr-edit-action-bar') && !target.closest('#gemini-ocr-settings-button, #gemini-ocr-global-anki-export-btn, #gemini-ocr-global-edit-btn')) {
            hideActiveOverlay();
        }
    }
    ...
    function handleBoxDelete(boxElement, sourceImage) {
        const data = ocrDataCache.get(sourceImage); if (!data) return;
    ...
    function handleMergeSelection(boxElement, overlay) {
        ...
        if (indexInSelection > -1) { currentSelection.splice(indexInSelection, 1); boxElement.classList.remove('selected-for-merge'); }
        else { currentSelection.push(boxElement); boxElement.classList.add('selected-for-merge'); }
    ...
    function finalizeMultipleMerge(selectedBoxes, sourceImage, overlay) {
        ...
        const indicesToDelete = new Set(); let newBoundingBox = null; let areAllVertical = true;
        selectedBoxes.sort((a, b) => { const rectA = a.getBoundingClientRect(); const rectB = b.getBoundingClientRect(); if (Math.abs(rectA.top - rectB.top) < 20) return rectA.left - rectB.left; return rectA.top - rectB.top; });
        const combinedTextParts = selectedBoxes.map(box => {
            indicesToDelete.add(box._ocrDataIndex); const b = box._ocrData.tightBoundingBox;
            ...
            if (!box.classList.contains('gemini-ocr-text-vertical')) { areAllVertical = false; } return box.dataset.fullText || box.textContent;
        });
        ...
        const originalData = ocrDataCache.get(sourceImage); const newData = originalData.filter((item, index) => !indicesToDelete.has(index));
        newData.push(newOcrItem); ocrDataCache.set(sourceImage, newData);
        selectedBoxes.forEach(box => box.remove());
        const newBoxElement = document.createElement('div');
        newBoxElement.className = 'gemini-ocr-text-box'; newBoxElement.innerHTML = newOcrItem.text.replace(/\u200B/g, "<br>"); newBoxElement.dataset.fullText = newOcrItem.text; newBoxElement._ocrData = newOcrItem; newBoxElement._ocrDataIndex = newData.length - 1; newBoxElement.style.whiteSpace = 'normal'; newBoxElement.style.textAlign = 'start';
        ...
        overlay.appendChild(newBoxElement); calculateAndApplyStylesForSingleBox(newBoxElement, sourceImage.getBoundingClientRect());
        activeMergeSelections.set(overlay, []); updateEditActionBar(overlay);
    }
    function handleOverlayInteraction(event) {
        const overlay = event.currentTarget; const clickedBox = event.target.closest('.gemini-ocr-text-box');
        if (!clickedBox) return; event.stopPropagation();
        const sourceImage = activeImageForExport; if (!sourceImage) return;
    ...
    function displayOcrResults(targetImg) {
        ...
        // [SYNCED] RTL-aware sorting from PC
        data.sort((a, b) => {
            const a_y = a.tightBoundingBox.y, b_y = b.tightBoundingBox.y;
            const a_x = a.tightBoundingBox.x, b_x = b.tightBoundingBox.x;
            if (Math.abs(a_y - b_y) < 0.05) return b_x - a_x;
            return a_y - b_y;
        });

        if (settings.autoMergeEnabled) { data = autoMergeOcrData(data); ocrDataCache.set(targetImg, data); }
        ...
        data.forEach((item, index) => {
            ...
            if (item.isMerged || item.text.includes("\u200B")) { ocrBox.style.whiteSpace = 'normal'; ocrBox.style.textAlign = 'start'; }
        ...
        resizeObserver.observe(targetImg); intersectionObserver.observe(targetImg);
    }
    ...
    function applyTheme() {
        ...
        const cssVars = `:root { --accent: ${theme.accent}; --background: ${theme.background}; --modal-header-color: rgba(${theme.accent}, 1); --ocr-dimmed-opacity: ${settings.dimmedOpacity}; --ocr-focus-scale: ${settings.focusScaleMultiplier}; }`;
    ...
    function createUI() {
        GM_addStyle(`
            ...
            .gemini-ocr-decoupled-overlay.is-inactive { opacity: 0; visibility: hidden; } /* Synced with PC */
            ...
            body.ocr-brightness-light .is-focused .gemini-ocr-text-box.selected-for-merge { background: rgba(var(--background), 1); color: rgba(var(--accent), 1); box-shadow: 0px 0px 0px 0.1em rgba(var(--background), 1), 0px 0px 0px 0.2em rgba(var(--accent), 1); }
            ...
            body.ocr-brightness-dark .is-focused .gemini-ocr-text-box.selected-for-merge { background: rgba(var(--accent), 1); color: #FFFFFF; box-shadow: 0px 0px 0px 0.1em rgba(var(--accent), 0.4), 0px 0px 0px 0.2em rgba(var(--background), 1); }
            .gemini-ocr-text-vertical { writing-mode: vertical-rl; text-orientation: upright; }
            /* [FIXED] No focus/zoom in edit mode */
            ...
            .solo-hover-mode.is-focused:not(.edit-mode-active):not(.has-manual-highlight) .gemini-ocr-text-box { opacity: 0; }
            /* [NEW] Edit Mode UI */
            ...
            .gemini-ocr-edit-action-bar .edit-action-merge { background-color: #3498db; }
            .gemini-ocr-edit-action-bar .edit-action-delete { background-color: #e74c3c; }
            .gemini-ocr-edit-action-bar button:disabled { background-color: #7f8c8d; opacity: 0.6; cursor: not-allowed; }
    ...
    function bindUIEvents() {
        ...
        UI.globalEditButton.addEventListener('click', (e) => {
            e.stopPropagation(); if (activeOverlay) {
                activeOverlay.classList.toggle('edit-mode-active'); UI.globalEditButton.classList.toggle('edit-active');
                if (!activeOverlay.classList.contains('edit-mode-active')) { activeMergeSelections.delete(activeOverlay); activeOverlay.querySelectorAll('.selected-for-merge').forEach(el => el.classList.remove('selected-for-merge')); } else { updateEditActionBar(activeOverlay); }
            }
        });
    ...
    async function init() {
        ...
        logDebug("Initializing MOBILE HYBRID render engine (Synced with PC).");
        ...
        setInterval(() => { for (const [img] of managedElements.entries()) { if (!img.isConnected) { logDebug("Detected disconnected image during periodic cleanup fallback."); fullCleanupAndReset(); setTimeout(reinitializeScript, 250); break; } } }, 5000);
    }

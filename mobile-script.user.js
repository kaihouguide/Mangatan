...
(function() {
    ...
    let settings = {
        ...
        sites: [{
            ...
            overflowFixSelector: '' // Mobile version uses parent-relative overlays, so this is not needed.
        }],
        debugMode: true,
    ...
    // --- Chapter List Observer (Synced with PC) ---
    ...
    function calculateAndApplyFontSizes(overlay, imgRect) {
        ...
        textBoxes.forEach(box => {
            ...
            let low = MIN_FONT_SIZE, high = 150, bestSize = MIN_FONT_SIZE;

            const isVertical = box.classList.contains('gemini-ocr-text-vertical');
            box.style.whiteSpace = isVertical ? '' : 'nowrap';
            if(isVertical) { measurementSpan.style.writingMode = 'vertical-rl'; measurementSpan.style.textOrientation = 'upright'; }
    ...
    function updateAllOverlays() {
        try {
            ...
            elementsToDelete.forEach(img => {
                ...
                if (state) { state.overlay.remove(); managedElements.delete(img); ocrDataCache.delete(img); logDebug(`Garbage collected overlay for detached image.`); }
            });
    ...
    async function batchProcessCurrentChapterFromURL() {
        ...
        if (!urlMatch) {
            ...
            btn.disabled = false; btn.textContent = "Pre-process Chapter";
            return;
        }
    ...
    function bindUIEvents() {
        Object.assign(UI, {
            ...
            serverUrlInput: document.getElementById('gemini-ocr-server-url'), imageServerUserInput: document.getElementById('gemini-image-server-user'), imageServerPasswordInput: document.getElementById('gemini-image-server-password'), ankiUrlInput: document.getElementById('gemini-ocr-anki-url'), ankiFieldInput: document.getElementById('gemini-ocr-anki-field'),
            debugModeCheckbox: document.getElementById('gemini-ocr-debug-mode'), interactionModeSelect: document.getElementById('ocr-interaction-mode'), activationModeSelect: document.getElementById('ocr-activation-mode'), proximityRadiusInput: document.getElementById('ocr-proximity-radius'), dimmedOpacityInput: document.getElementById('ocr-dimmed-opacity'),
        ...
        UI.saveBtn.addEventListener('click', async () => {
            const newSettings = {
                ...
                interactionMode: UI.interactionModeSelect.value, activationMode: UI.activationModeSelect.value, textOrientation: UI.textOrientationSelect.value,
                colorTheme: UI.colorThemeSelect.value, proximityRadius: parseInt(UI.proximityRadiusInput.value, 10) || 150,

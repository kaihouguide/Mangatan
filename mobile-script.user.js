// ==UserScript==
// @name         Automatic Content OCR (v23.M.4 - Engine-Refactor)
// @namespace    http://tampermonkey.net/
// @version      23.4.0
// @description  Server-side OCR with a mobile-optimized UI. Features refactored orientation and font-sizing engines.
...
(function() {
    ...
    let debugLog = [];
    const SETTINGS_KEY = 'gemini_ocr_settings_v23_M4_engine'; // Updated settings key
    ...
    const logDebug = (message) => {
        ...
        console.log(`[OCR v23.M.4-Engine] ${logEntry}`);
        debugLog.push(logEntry);
        document.dispatchEvent(new CustomEvent('ocr-log-update'));
    };

    // --- TOUCH INTERACTION LOGIC (Mobile Specific - Unchanged) ---
    ...
    // --- Core Observation & Processing Logic ---
    ...
    // --- MODIFIED: processImage now correctly parses server response ---
    function processImage(img, sourceUrl) {
        if (ocrDataCache.has(img) && ocrDataCache.get(img) !== 'pending') {
            displayOcrResults(img); // Display from session cache if available
            return;
        }
        ...
        GM_xmlhttpRequest({
            ...
            onload: (res) => {
                try {
                    ...
                    ocrDataCache.set(img, ocrResultForThisImage);
    ...
    // --- NEW: Robust Orientation Detection Helper ---
    function determineIfVertical(item, textOrientationSetting) {
        ...
        if (textOrientationSetting === 'smart') {
            if (hasReliableAngle) { return Math.abs(item.orientation) > 45; }
            if (box.height > 0) { return (box.width / box.height) > 1.5; } // Vertical text is wider than it is tall
            return false;
        }
        return false;
    }

    // --- MODIFIED: displayOcrResults now uses the new helper ---
    function displayOcrResults(targetImg) {
        ...
        // Sort by reading order (top-to-bottom, then right-to-left for manga)
    ...
    // --- REFACTORED: Accurate Font Calculation for Mobile ---
    function calculateAndApplyFontSizes(overlay, imgRect) {
        ...
        overlay.querySelectorAll('.gemini-ocr-text-box').forEach(box => {
            ...
            // Calculate available space inside the box based on its percentage dimensions and the image's actual size
            const availableWidth = (parseFloat(boxStyle.width) / 100) * imgRect.width - 8; // 4px padding on each side
            const availableHeight = (parseFloat(boxStyle.height) / 100) * imgRect.height - 8; // 4px padding on each side

            if (availableWidth <= 0 || availableHeight <= 0) return;

            // Configure the measurement tool
            ...
            while (low <= high) {
                ...
                if (measurementSpan.offsetWidth <= availableWidth && measurementSpan.offsetHeight <= availableHeight) {
                    bestSize = mid; low = mid + 1; // It fits, try larger
                } else {
    ...
    // --- REFACTORED UPDATE LOOP (Unchanged logic, but now supports better font calc) ---
    function updateAllOverlays() {
        try {
            ...
            else { overlayUpdateRunning = false; logDebug("All overlays removed. Halting update loop."); }
        }
    }

    // --- BATCH & SERVER-SIDE PROCESSING (Unchanged) ---
    ...
    // --- UI Creation & Event Binding (Largely Unchanged) ---
    function createUI() {
        GM_addStyle(`
            /* Unchanged mobile-optimized CSS ... */
        ...
        document.body.insertAdjacentHTML('beforeend', `
            <!-- Unchanged mobile-optimized HTML ... -->
              ...
              <div class="gemini-ocr-modal-container">
                ...
                <div class="gemini-ocr-modal-content">
                    ...
                    <h3>Interaction & Display</h3><div class="gemini-ocr-settings-grid">
                        <label for="ocr-activation-mode">Activation Gesture:</label><select id="ocr-activation-mode"><option value="longPress">Long Press</option><option value="doubleTap">Double Tap</option></select>

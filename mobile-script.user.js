...
(function() {
    ...
    const logDebug = (message) => { 
        ...
        console.log(`[OCR v23.9.6-Hybrid] ${logEntry}`); 
    ...
    function triggerOverlayToggle(targetImg) { 
        ...
        if (overlayState?.overlay) { 
            if (overlayState.overlay === activeOverlay) { 
                ...
                showOverlay(overlayState.overlay, targetImg); 
            } 
        } 
    }
    ...
    function handleTouchStart(event) { 
        if (event.touches.length > 1) { 
            longPressState.valid = false; return; 
        } 
    ...
    function handleContextMenu(event) { 
        if (settings.activationMode === 'longPress' && longPressState.valid && event.target === longPressState.target) { 
            ...
            longPressState.valid = false; return false; 
        } 
    }
    ...
    function activateScanner() { 
        logDebug("Activating Best-Fit scanner...");
        ...
        GM_xmlhttpRequest({ 
            ...
            onload: (res) => { 
                try { 
                    ...
                    ogDebug(`OCR success for ...${sourceUrl.slice(-30)}`); 
                    displayOcrResults(img); 
                } catch (e) { 
            ...
            onerror: (res) => { 
                logDebug(`Connection error. Status: ${res.status}`); 
                ocrDataCache.delete(img); 
            }, 
            ontimeout: () => { 
    ...
    async function exportImageToAnki(targetImg) { 
        ...
        try { 
            ...
            if (error.message.includes("SecurityError") || error.message.includes("tainted")) { 
                alert(`Anki Export Failed: Canvas security error due to CORS policy.`); 
            } 
            else { 
    ...
    function bindUIEvents() { 
        Object.assign(UI, { 
            ...
            proximityRadiusInput: document.getElementById('ocr-proximity-radius'), 
        ...
        UI.globalAnkiButton.addEventListener('click', async () => {
            if (!activeImageForExport) {
                alert("No active image selected for export."); 
                return;
            }
        ...
        UI.saveBtn.addEventListener('click', async () => {
            const newSettings = {
                ...
                activationMode: UI.activationModeSelect.value, 
                ...
                focusScaleMultiplier: parseFloat(UI.focusScaleMultiplierInput.value) || 1.2, 
                sites: UI.sitesConfigTextarea.value.split('\n').filter(line => line.trim()).map(line => {
                    const parts = line.split(';').map(s => s.trim());
                    return {
                        urlPattern: parts[0] || '',
                        overflowFixSelector: '', 
                        imageContainerSelectors: parts.slice(1).filter(s => s) 
                    };
                })
            };
            try {

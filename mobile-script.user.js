// ==UserScript==
// @name         Automatic Content OCR (v23.9.3-Performance-Refactor)
// @namespace    http://tampermonkey.net/
// @version      23.9.3
// @description  Server-side OCR with a mobile-optimized UI. Performance-refactored to eliminate lag using ResizeObserver and IntersectionObserver.
...
(function() {
    ...
    let debugLog = [];
    const SETTINGS_KEY = 'gemini_ocr_settings_v23_M9_bestfit'; // Using the same key to keep settings
    ...
    const logDebug = (message) => { if (!settings.debugMode) return; const timestamp = new Date().toLocaleTimeString(); const logEntry = `[${timestamp}] ${message}`; console.log(`[OCR v23.9.3-Perf] ${logEntry}`); debugLog.push(logEntry); document.dispatchEvent(new CustomEvent('ocr-log-update')); };

    // --- PERFORMANCE REFACTOR: Observers & Throttling ---
    let resizeObserver;
    let intersectionObserver;
    let visibleImages = new Set();
    let scrollTimeout = null;

    // This function now runs only when an element's size actually changes.
    const handleResize = (entries) => {
        for (const entry of entries) {
            const img = entry.target;
            const state = managedElements.get(img);
            if (state) {
                logDebug(`Resize detected for ...${img.src.slice(-30)}`);
                updateSingleOverlay(img, state, entry.contentRect);
            }
        }
    };

    // This function efficiently tracks visibility.
    const handleIntersection = (entries) => {
        for (const entry of entries) {
            ...
            if (entry.isIntersecting) {
                ...
                // Initial update on becoming visible
                updateSingleOverlay(img, state);
            } else {
    ...
    // Throttled scroll/resize handler to prevent performance drops.
    const handleScrollOrResize = () => {
        ...
        scrollTimeout = setTimeout(() => {
            for (const img of visibleImages) {
                const state = managedElements.get(img);
                if (state) {
                    // We only need to update position, not recalculate styles
            ...
            scrollTimeout = null;
        }, 100); // Throttle to 10fps for position updates during scroll.
    };

    // Low-frequency cleanup to prevent memory leaks from removed images.
    ...
    /**
     * Centralized update function for a single overlay.
     * Replaces the need for a global `requestAnimationFrame` loop.
     */
    function updateSingleOverlay(img, state, rect = null) {
        if (!rect) {
        ...
        // The expensive style calculation only runs if the dimensions have changed.
        if (state.lastWidth !== rect.width || state.lastHeight !== rect.height) {
            ...
            logDebug(`Recalculated styles for ...${img.src.slice(-30)} (W: ${rect.width.toFixed(0)}, H: ${rect.height.toFixed(0)})`);
        }
    }
    ...
    // --- Overlay Management (Unchanged core logic, only visibility) ---
    ...
    // --- CORRECTED & OPTIMIZED Best-Fit Rendering Engine (Unchanged) ---
    function calculateAndApplyOptimalStyles_Optimized(overlay, imgRect) {
        ...
        const calculationJobs = boxes.map(box => ({ box: box, text: box.textContent || '', availableWidth: (parseFloat(box.style.width) / 100) * imgRect.width - 8, availableHeight: (parseFloat(box.style.height) / 100) * imgRect.height - 8 }));
        const styleResults = [];
        ...
        for (const result of styleResults) { result.box.style.fontSize = result.fontSize; result.box.classList.toggle('gemini-ocr-text-vertical', result.isVertical); }
        measurementSpan.style.writingMode = 'horizontal-tb';
    }
    ...
    function displayOcrResults(targetImg) {
        ...
        // NOTE: 'gemini-ocr-decoupled-overlay' is now absolutely positioned relative to the document body.
        ...
        // Hook into the new efficient observers.
        resizeObserver.observe(targetImg);
        intersectionObserver.observe(targetImg);

        // Perform the initial placement and style calculation.
        updateSingleOverlay(targetImg, state);
    }


    // --- BATCH, UI, Anki, and Initialization (Unchanged, except init()) ---
    ...
    async function init() {
        ...
        // --- PERFORMANCE REFACTOR INITIALIZATION ---
        logDebug("Initializing performance-optimized observers.");
        resizeObserver = new ResizeObserver(handleResize);
        intersectionObserver = new IntersectionObserver(handleIntersection, { rootMargin: '200px 0px' }); // Pre-load/check images 200px beyond the viewport
        window.addEventListener('scroll', handleScrollOrResize, { passive: true });
        window.addEventListener('resize', handleScrollOrResize);
        setInterval(periodicCleanup, 10000); // Run cleanup every 10 seconds.
        // --- END REFACTOR INITIALIZATION ---

...
// @version      24.5.11-M-Unified-Theming
// @description  Adds a mobile-optimized OCR overlay with a stable hybrid rendering engine, advanced group-based editing, automatic merging, and a new dark mode.
// @author       1Selxo (PC Base by 1Selxo, Mobile Port & Unified Theming by Gemini)
// @match        *://127.0.0.1*/*
// @grant        GM_setValue
// @grant        GM_getValue
// @grant        GM_addStyle
...
(function() {
    ...
    let settings = {
        ...
        soloHoverMode: false,
        colorTheme: 'blue',
        brightnessMode: 'light', // 'light' or 'dark'
        // Auto-Merge Settings
    ...
    let debugLog = [];
    const SETTINGS_KEY = 'gemini_ocr_settings_v24_mobile_unified_themes';
    ...
    function displayOcrResults(targetImg) {
        ...
        dataGroups.forEach((groupData) => {
            ...
            groupData.forEach((item) => {
                ...
                ocrBox.textContent = item.text.replace(/[!?]{2,}/g, (match) => match.includes('?') ? '?' : '!');
        ...
        overlay.addEventListener('click', (e) => handleOverlayInteraction(e, targetImg));
    ...
    function hideActiveOverlay() {
        ...
        activeOverlay.querySelectorAll('.manual-highlight, .selected-for-merge').forEach(el => el.classList.remove('manual-highlight', 'selected-for-merge'));
        ...
        UI.editorBar?.classList.add('is-hidden');
        activeOverlay = null;
    ...
    function handleGlobalTap(event) {
        ...
        if (!activeOverlay.contains(target) && !target.closest('#gemini-ocr-settings-button, #gemini-ocr-global-anki-export-btn, #gemini-ocr-global-edit-btn, #gemini-ocr-editor-bar')) {
            hideActiveOverlay();
        }
    }
    function handleOverlayInteraction(e, sourceImage) {
        const overlay = e.currentTarget;
        const clickedGroup = e.target.closest('.gemini-ocr-group');

        if (document.body.classList.contains('ocr-edit-mode-active')) {
            if (clickedGroup) {
                e.stopPropagation();
    ...
    function handleGroupDelete(sourceImage) {
        if (!activeOverlay) return;
        const selection = activeMergeSelections.get(activeOverlay);
        if (!selection || selection.length === 0) { alert("No groups selected to delete."); return; }

        logDebug(`Deleting ${selection.length} selected groups.`);
        const data = ocrDataCache.get(sourceImage);
        if (!data || !Array.isArray(data)) return;

        const boxesToDelete = new Set(selection.flatMap(g => Array.from(g.querySelectorAll('.gemini-ocr-text-box'))));
        const dataToRemove = new Set(Array.from(boxesToDelete).map(box => box._ocrData));
        const updatedData = data.filter(item => !dataToRemove.has(item));
        ocrDataCache.set(sourceImage, updatedData);

        selection.forEach(group => group.remove());
        activeMergeSelections.delete(activeOverlay);
        logDebug(`Cache updated. Items remaining: ${updatedData.length}`);
    }
    function handleMergeSelection(groupElement, overlay) {
        ...
        if (indexInSelection > -1) {
            ...
            logDebug(`Group deselected. Total selected: ${currentSelection.length}`);
        } else {
            currentSelection.push(groupElement);
            groupElement.classList.add('selected-for-merge');
            logDebug(`Group selected. Total selected: ${currentSelection.length}`);
        }
        if (currentSelection.length === 0) {
            activeMergeSelections.delete(overlay);
        }
    }
    function finalizeManualMerge() {
        if (!activeOverlay) return;
        const selectedGroups = activeMergeSelections.get(activeOverlay);
        if (!selectedGroups || selectedGroups.length < 2) {
            logDebug("Grouping cancelled: less than 2 groups were selected.");
        ...
        activeOverlay.appendChild(newGroupWrapper);
    ...
    function createUI() {
        GM_addStyle(`
            ...
            /* --- Overlay & Interaction --- */
            .gemini-ocr-decoupled-overlay { position: fixed; z-index: 9998; opacity: 0; visibility: hidden; pointer-events: none; transition: opacity 0.2s, visibility 0.2s; -webkit-tap-highlight-color: transparent; }
            .gemini-ocr-decoupled-overlay.is-focused { opacity: 1; visibility: visible; }
            ...
            body.ocr-brightness-light:not(.ocr-theme-minimal):not(.ocr-edit-mode-active) .is-focused .gemini-ocr-text-box.manual-highlight { background: rgba(var(--background), 1); color: rgba(var(--accent), 1); box-shadow: 0px 0px 0px 0.1em rgba(var(--background), 1), 0px 0px 0px 0.2em rgba(var(--accent), 1); }
            ...
            body.ocr-brightness-dark:not(.ocr-theme-minimal):not(.ocr-edit-mode-active) .is-focused .gemini-ocr-text-box.manual-highlight { background: rgba(var(--accent), 1); color: #FFFFFF; box-shadow: 0px 0px 0px 0.1em rgba(var(--accent), 0.4), 0px 0px 0px 0.2em rgba(var(--background), 1); }

            /* Common Styles */
            body:not(.ocr-theme-minimal) .gemini-ocr-text-vertical { writing-mode: vertical-rl; text-orientation: upright; }
            body:not(.ocr-theme-minimal):not(.ocr-edit-mode-active) .is-focused .gemini-ocr-text-box.manual-highlight { z-index: 1; transform: scale(var(--ocr-focus-scale)); overflow: visible; }

            /* Dimming */
            body:not(.ocr-theme-minimal) .is-focused.has-manual-highlight .gemini-ocr-group:not(:has(.manual-highlight)) { opacity: ${settings.dimmedOpacity}; }
            .solo-hover-mode.is-focused .gemini-ocr-group { opacity: 0; } .solo-hover-mode.is-focused .gemini-ocr-group:has(.manual-highlight), .solo-hover-mode.is-focused .gemini-ocr-group.selected-for-merge { opacity: 1; }

            /* Minimal Theme (Untouched) */
            body.ocr-theme-minimal .gemini-ocr-text-box { background: transparent !important; color: transparent !important; border: none !important; backdrop-filter: none !important; text-shadow: none !important; box-shadow: none !important; transform: none !important; }

            /* --- Mobile Editor Mode --- */
            .gemini-ocr-group.selected-for-merge { outline: 3px solid #f1c40f !important; outline-offset: 2px; box-shadow: 0 0 12px #f1c40f; }
            body.ocr-edit-mode-active .is-focused .gemini-ocr-group:not(.selected-for-merge) { opacity: 0.4; }
            body.ocr-edit-mode-active .is-focused .gemini-ocr-group { transition: opacity 0.1s; }
            ...
            .is-hidden { opacity: 0 !important; visibility: hidden !important; pointer-events: none !important; transform: scale(0.5) !important; }

            /* Editor Bar */
            #gemini-ocr-editor-bar { position: fixed; bottom: 0; left: 0; right: 0; z-index: 2147483647; background: rgba(20,25,30,0.8); backdrop-filter: blur(8px); display: flex; justify-content: space-around; padding: 10px; padding-bottom: calc(10px + env(safe-area-inset-bottom)); transition: transform 0.3s ease-in-out; transform: translateY(100%); }
            #gemini-ocr-editor-bar.is-hidden { transform: translateY(100%); }
            body.ocr-edit-mode-active #gemini-ocr-editor-bar { transform: translateY(0); }
            #gemini-ocr-editor-bar button { font-size: 1rem; color: white; background: #555; border: none; border-radius: 8px; padding: 12px 20px; font-weight: bold; }
            #gemini-ocr-editor-bar button#gemini-ocr-editor-delete { background-color: #c0392b; }
            #gemini-ocr-editor-bar button#gemini-ocr-editor-merge { background-color: #2980b9; }
    ...
    function bindUIEvents() {
        ...
        UI.globalEditButton.addEventListener('click', (e) => {
            e.stopPropagation();
            if (activeOverlay) {
                const isEditing = document.body.classList.toggle('ocr-edit-mode-active');
                UI.globalEditButton.classList.toggle('edit-active', isEditing);
                if (!isEditing) {
        ...
        UI.editorMergeBtn.addEventListener('click', (e) => { e.stopPropagation(); finalizeManualMerge(); });
        UI.editorDeleteBtn.addEventListener('click', (e) => { e.stopPropagation(); handleGroupDelete(activeImageForExport); });
    ...
    async function init() {
        ...
        logDebug("Initializing MOBILE HYBRID render engine with Unified Theming.");

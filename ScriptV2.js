...
// @version      2.9
// @description  Reliably positions OCR text on all pages by waiting for images to load, while using efficient lookahead batch processing.
...
(function() {
    ...
    const main = async () => {
        ...
        // --- 4. CORE LOGIC (Restored Batch Processing) ---
        ...
        // MODIFICATION: Logic restored to handle batching correctly.
        function discoverAndQueueImages(container) {
            ...
            if (imagesScheduledForOcr === 0 && imageQueue.length > 0) {
        ...
        function primeImage(img, imageNumber) {
            logDebug(`Priming image #${imageNumber}. Adding 'load' event listener.`);
        ...
        const processImage = (img, imageNumber) => {
            ...
            if (realSrc && realSrc.startsWith('http')) {
                ...
                logDebug(`Image #${imageNumber} has no valid src yet. It will be processed when its 'src' is set.`);
                const attrObserver = new MutationObserver(() => { const updatedSrc = img.dataset.src || img.src; if (updatedSrc && updatedSrc.startsWith('http')) { attrObserver.disconnect(); processImage(img, imageNumber); } });
